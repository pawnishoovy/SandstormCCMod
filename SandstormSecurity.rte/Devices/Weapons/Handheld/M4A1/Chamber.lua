function Create(self)
	...
	self.sharpAimSounds.In = CreateSoundContainer("SharpAimIn M4A1", "SandstormSecurity.rte");
	self.sharpAimSounds.Out = CreateSoundContainer("SharpAimOut M4A1", "SandstormSecurity.rte");	
	
	self.addSounds = {["Start"] = nil, ["Loop"] = nil};
	self.addSounds.Start = CreateSoundContainer("AddStart M4A1", "SandstormSecurity.rte");
	self.addSounds.Loop = CreateSoundContainer("Add M4A1", "SandstormSecurity.rte");
	
	self.bassSounds = {["Loop"] = nil};
	self.bassSounds.Loop = CreateSoundContainer("Bass M4A1", "SandstormSecurity.rte");
	
	self.mechSounds = {["Start"] = nil, ["Loop"] = nil};
	self.mechSounds.Start = CreateSoundContainer("MechStart M4A1", "SandstormSecurity.rte");
	self.mechSounds.Loop = CreateSoundContainer("Mech M4A1", "SandstormSecurity.rte");
	...
	self.reloadPrepareSounds.MagOut = CreateSoundContainer("MagOutPrepare M4A1", "SandstormSecurity.rte");
	self.reloadPrepareSounds.MagIn = CreateSoundContainer("MagInPrepare M4A1", "SandstormSecurity.rte");
	...
	self.reloadAfterSounds.MagOut = CreateSoundContainer("MagOut M4A1", "SandstormSecurity.rte");
	self.reloadAfterSounds.MagIn = CreateSoundContainer("MagIn M4A1", "SandstormSecurity.rte");
	self.reloadAfterSounds.MagHit = CreateSoundContainer("MagHit M4A1", "SandstormSecurity.rte");
	self.reloadAfterSounds.BoltForwardRelease = CreateSoundContainer("BoltRelease M4A1", "SandstormSecurity.rte");
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack M4A1", "SandstormSecurity.rte");
	self.reloadAfterSounds.BoltForward = CreateSoundContainer("BoltForward M4A1", "SandstormSecurity.rte");
	...
	self.boltReleasePrepareDelay = 700;
	self.boltReleaseAfterDelay = 400;
...
function Update(self)
	...
	if self:IsReloading() then
		...
		elseif self.reloadPhase == 3 then
			self.Frame = 2;
			self.reloadDelay = self.boltReleasePrepareDelay;
			self.afterDelay = self.boltReleaseAfterDelay;
			
			self.prepareSound = nil
			self.prepareSoundLength = 0
			self.afterSound = self.reloadAfterSounds.BoltForwardRelease;
		...
		if self.reloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			if self.reloadTimer:IsPastSimMS(self.reloadDelay + self.afterDelay) then
				...
				if self.chamberOnReload and self.reloadPhase == 2 then
					local name = ToActor(self.parent).PresetName;
					if name == "Sandstorm Insurgency Light" or name == "Sandstorm Insurgency Heavy" then
						self.reloadPhase = 4;
					else
						self.reloadPhase = self.reloadPhase + 1;
					end
				elseif self.reloadPhase == 2 or self.reloadPhase == 3 or self.reloadPhase == 5 then
					self.ReloadTime = 0;
					self.reloadPhase = 0;
					self.phaseOnStop = nil;
				else
